...
class CustomerMCCarmeraUI: UIViewController {
    ...
    func prepareLight(index:Int)
    {
        ...
        self.camera.focusAt(point: CGPoint(x: ScreenWidth/2,y: ScreenHeight/2), inView: self.previewView)
        self.prepared = true
    }
    ...
    func drawPreiview(index:Int){
        ...
        if self.step == 2{
            self.imgView.image = UIImage.init(data: self.imgData[self.current]);
